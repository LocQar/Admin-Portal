...
import { Search, Users2, UserCheck, UserX, QrCode, CreditCard, Phone, ToggleLeft, ToggleRight, Upload, Download, Package, Plus, Star, X, Trash2, Eye, Edit, LayoutGrid, List, ChevronRight, ArrowUpRight, ArrowDownRight } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[[1, 'Active', theme.status.success], [0, 'Disabled', '#A8A29E']].map(([v, l, c]) => (
...
export const CouriersPage = ({ addToast, packages = [] }) => {
  ...
  const filteredCouriers = useMemo(() => {
    ...
    return list;
  }, [couriers, search, statusFilter, terminalFilter, sortBy, sortDir]);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => fileInputRef.current?.click()} className="btn-outline flex items-center gap-2 px-4 py-2 rounded-xl border text-sm" style={{ borderColor: theme.border.primary, color: theme.text.secondary }}>
            <Upload size={16} /> Import
          </button>
          <button onClick={exportToCSV} className="btn-outline flex items-center gap-2 px-4 py-2 rounded-xl border text-sm" style={{ borderColor: theme.border.primary, color: theme.text.secondary }}>
            <Download size={16} /> Export
          </button>
          <button onClick={() => setDrawerCourier({})} className="btn-primary flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium" style={{ backgroundColor: theme.accent.primary, color: theme.accent.contrast }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          ['Active', activeCouriers, UserCheck, theme.status.success],
          ['Disabled', disabledCouriers, UserX, '#A8A29E'],
          ['Avg Rating', avgRating, Star, theme.status.warning],
      ...
      {view === 'grid' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredCouriers.map(courier => {
            ...
            return (
              <GlassCard key={courier.id} hover onClick={() => setViewCourier(courier)} className="group space-y-3" style={{ padding: '1rem' }}>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: `1px solid ${theme.border.primary}` }}>
                {[
                  { label: 'Courier', field: 'name', cls: '' },
                  { label: 'Terminal / Zone', field: 'terminal', cls: 'hidden md:table-cell' },
                  { label: 'Phone', field: 'phone', cls: 'hidden md:table-cell' },
                  { label: 'Card / QR', field: null, cls: 'hidden lg:table-cell' },
                  { label: 'Deliveries', field: 'totalDeliveries', cls: 'hidden lg:table-cell' },
                  { label: 'Rating', field: 'rating', cls: 'hidden lg:table-cell' },
                  { label: 'Status', field: 'status', cls: '' },
                ].map(col => (
                  <th key={col.label} className={`text-left p-4 text-xs font-semibold uppercase cursor-pointer select-none ${col.cls}`} style={{ color: sortBy === col.field ? theme.accent.primary : theme.text.muted }}>
                    {col.field ? (
                      <button onClick={() => toggleSort(col.field)} className="flex items-center gap-1 hover:opacity-80">
                        {col.label}
              ...
              {filteredCouriers.map(courier => {
                ...
                return (
                  <tr key={courier.id} className="hover:bg-white/5 cursor-pointer" style={{ borderBottom: `1px solid ${theme.border.primary}` }} onClick={() => setViewCourier(courier)}>
                    ...
                    <td className="p-4" onClick={e => e.stopPropagation()}>
                      <button onClick={() => handleToggleStatus(courier.id)} className="inline-flex items-center gap-1.5 text-xs" style={{ color: courier.status === 1 ? theme.status.success : '#A8A29E' }}>
          ...
          <div className="absolute inset-y-0 right-0 w-full sm:w-[400px] border-l shadow-2xl flex flex-col" style={{ backgroundColor: theme.bg.secondary, borderColor: theme.border.primary }}>
            <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: theme.border.primary }}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { setDrawerCourier(viewCourier); setViewCourier(null); }} className="btn-outline px-3 py-1.5 rounded-lg text-xs border" style={{ borderColor: theme.border.primary, color: theme.text.secondary }}>Edit</button>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  ['Rating', viewCourier.rating ? `${viewCourier.rating} ★` : '—', theme.status.warning],
                  ['Active Pkgs', packages.filter(p => p.courier?.id === viewCourier.id).length, theme.status.success],
                ].map(([l, v, c]) => (
            ...
            <div className="p-4 border-t flex gap-3" style={{ borderColor: theme.border.primary }}>
              <button onClick={() => { handleToggleStatus(viewCourier.id); setViewCourier(prev => ({ ...prev, status: prev.status === 1 ? 0 : 1 })); }} className="flex-1 py-2.5 rounded-xl border text-sm" style={{ borderColor: theme.border.primary, color: viewCourier.status === 1 ? '#A8A29E' : theme.status.success }}>
                {viewCourier.status === 1 ? 'Disable Courier' : 'Enable Courier'}
              </button>
              <button onClick={() => { setDeleteConfirm(viewCourier); setViewCourier(null); }} className="py-2.5 px-4 rounded-xl border text-sm text-red-400" style={{ borderColor: `${theme.status.error}40` }}>
                <Trash2 size={15} />
              </button>
            </div>
          </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center" onClick={() => setDeleteConfirm(null)}>
          ...
          <GlassCard className="relative w-full max-w-sm space-y-4" style={{ padding: '1.5rem' }} onClick={e => e.stopPropagation()}>
