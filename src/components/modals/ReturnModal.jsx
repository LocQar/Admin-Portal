...
export const ReturnModal = ({ isOpen, onClose, pkg }) => {
  ...
  const handleSubmit = () => {
    if (!returnReason) {
      // addToast({ type: 'warning', message: 'Please select a return reason' });
      return;
    }
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold flex items-center gap-2" style={{ color: theme.text.primary }}>
            <PackageX size={20} className="text-red-500" /> Return Package
          </h2>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {RETURN_REASONS.map(r => (
                <button
                  ...
                  style={{
                    backgroundColor: returnReason === r.id ? 'rgba(239, 68, 68, 0.08)' : theme.bg.tertiary,
                    color: returnReason === r.id ? '#ef4444' : theme.text.secondary,
                    border: returnReason === r.id ? '1px solid rgba(239, 68, 68, 0.3)' : `1px solid ${theme.border.primary}`
                  }}
                >
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[['warehouse', 'Warehouse', Warehouse], ['sender', 'Sender', ArrowUpRight]].map(([k, l, Icon]) => (
                <button
                  key={k}
                  onClick={() => setReturnMethod(k)}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm"
                  style={{
                    backgroundColor: returnMethod === k ? theme.accent.light : theme.bg.tertiary,
                    color: returnMethod === k ? theme.accent.primary : theme.text.secondary,
                    border: returnMethod === k ? `1px solid ${theme.accent.border}` : `1px solid ${theme.border.primary}`
                  }}
                >
                  <Icon size={16} /> {l}
                </button>
          ...
          <div
            ...
            style={{ backgroundColor: theme.bg.tertiary, border: `1px solid ${theme.border.primary}` }}
          >
            <div>
            ...
            <div className="flex items-center gap-3">
              <span
                className={`text-sm font-semibold ${waiveReturn ? 'line-through opacity-50' : ''}`}
                style={{ color: theme.text.primary }}
              >
                GH₵ 8.00
              </span>
              <button
                onClick={() => setWaiveReturn(!waiveReturn)}
                className="px-2.5 py-1 rounded-lg text-xs"
                style={{
                  backgroundColor: waiveReturn ? 'rgba(16, 185, 129, 0.1)' : theme.bg.tertiary,
                  color: waiveReturn ? '#10b981' : theme.text.muted,
                  border: waiveReturn ? '1px solid rgba(16, 185, 129, 0.3)' : `1px solid ${theme.border.primary}`
                }}
              >
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-white text-sm flex items-center justify-center gap-2 bg-red-500"
          >
